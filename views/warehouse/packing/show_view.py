...
class PackingShowView(tk.Frame):
    ...
    def create_confirmed_orders_table(self):
        ...
        self.confirmed_tree = ttk.Treeview(self.confirmed_orders_frame, columns=columns, show="headings")
        for col in columns:
            ...
            # Inicialmente establecemos dirección ascendente para cada columna
    ...
    def fetch_process_detail(self):
        ...
        # Actualizar los productos pendientes de la orden actual
        self.pending_products = []
        if self.pending_process_order:
            products = self.pending_process_order.get("PackingProcessOrderProduct", [])
            for item in products:
                product = item.get("product", {})
                self.pending_products.append({
    ...
    def update_confirmed_orders_table(self, confirmed):
        ...
        self.current_page = 1
        self.total_pages = math.ceil(len(self.confirmed_orders_data) / self.page_size) if self.confirmed_orders_data else 1
